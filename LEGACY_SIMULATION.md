...
> 本页保留旧 `start_sim_4uav.sh`、`run_swarm.sh`、`goal_nav.launch.py` 和
> PX4 1.15.4 的操作记录。它使用 GPS/真值检测或先验场地图，不代表当前视觉
> 算法闭环。新用户请从 [README.md](README.md) 开始；不要混用两套入口。
...
```bash
git clone https://github.com/PingTIKES/origin_drone_27.git   # 或解压 zip
cd origin_drone_27        # 即 ROS 2 工作空间
...
- **场地网格版本**：历史发行包曾使用单层加工网格（md5
  `19079837e87223fcc745de9620966053`），也曾出现双层重叠版（md5
  `fa41fd76...`）。当前 Git 克隆后由 `fetch_field_model.sh` 下载的是固定上游提交的
  原始网格（md5 `440a0d88aadfbc3dafa5604cf50a5071`），不能把上述历史校验值
  当作下载失败。碰撞异常时应保留实际网格校验值和 `/tmp/gz_server.log` 排查。
...
| 场地模型缺失（世界只有几个停机坪/目标柱） | 场地网格没装上：确认 `worlds/models/rmuc_2025/meshes/rmuc_2025.stl` 存在（缺失时脚本会自动下载）；Gazebo 直接加载工作空间这份文件，启动日志会打印其 md5。当前下载的上游原始网格参考值为 `440a0d88aadfbc3dafa5604cf50a5071`，历史加工版校验值不同。 |
